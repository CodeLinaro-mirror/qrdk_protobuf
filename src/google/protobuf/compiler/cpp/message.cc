...
void MessageGenerator::GenerateSourceDefaultInstance(io::Printer* p) {
  ...
  auto section_name =
      MaybeDedicateSection(descriptor_, options_, index_in_file_messages_);
  ...
  if (IsFileDescriptorProto(descriptor_->file(), options_)) {
    ...
    p->Emit({{"section",
              [&] {
                if (!section_name.has_value()) return;
            ...
            R"cc(
...
#ifdef PROTOBUF_MESSAGE_GLOBALS
                      MessageGlobalsBase(
                          $Msg$::InternalGenerateClassData_(
              ...
              PROTOBUF_ATTRIBUTE_NO_DESTROY PROTOBUF_CONSTINIT$ dllexport_decl$
                  PROTOBUF_ATTRIBUTE_INIT_PRIORITY1 $section$ $globals_type$
                      $globals$;
            )cc");

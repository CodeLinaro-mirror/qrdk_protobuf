...
bool MustMangleName(const Desc& desc) {
  ...
  // matches the name without the 'View' suffix. If so, append an extra '_'
  // character on the end of the gencode name. The reason we special case mangle
  // this is to avoid breakages from the View type of the message when the
  // .proto file is following this AIP:

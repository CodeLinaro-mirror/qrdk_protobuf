...
TEST_F(RustGeneratorTest, KeepsReexportForSameFileDuplicateSymbol) {
  CreateTempFile("a.proto", R"schema(
    syntax = "proto2";
    package pkg_a;
    message Data {
...
TEST_F(RustGeneratorTest, KeepsReexportForSameFileDuplicateExtensionConst) {
  CreateTempFile("a.proto", R"schema(
    syntax = "proto2";
    package pkg_a;
    message Target { extensions 100 to 200; }
    extend Target {
      optional int32 shared = 100;
      optional int32 SHARED = 101;
    })schema");
  RunProtoc(
      "protocol_compiler --proto_path=$tmpdir "
      "--rust_out=$tmpdir "

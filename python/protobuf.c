...
#ifdef ENABLE_MUTEX
#define PYUPB_MUTEX_INITIALIZER {PTHREAD_MUTEX_INITIALIZER}
#else
#define PYUPB_MUTEX_INITIALIZER \
  {                             \
  }
#endif

static upb_Arena* PyUpb_NewArena(void);
static FreeThreadingMutex obj_cache_mutex = PYUPB_MUTEX_INITIALIZER;
